...
function create() {
    ...
    // background.scale.setTo(1.25, 7/6);
    ...
    ledge = platforms.create(500, 600, 'ground');
    ledge.body.moves = false;
    ...
    lastLedge = platforms.create(150, 450, 'ground');
    lastLedge.body.moves = false;
    lastLedge.scale.setTo(.5, 1);
    lastLedge.body.immovable = true;
...
function toEndAndBack(child) {
    ...
    if (child.deltaX > 0) {
        if (child.body.right < game.world.bounds.width) {
          child.x += incriment
        } else {
          child.x -= incriment
        }
    } else {
        if (child.body.x > game.world.bounds.x) {
          child.x -= incriment
        } else {
          child.x += incriment
        }
